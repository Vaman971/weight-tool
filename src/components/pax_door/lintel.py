"""PAX door lintel component builders.
...
class DoorLintel(GenericVolumetricBeam):
    """Build PAX door lintels with shared volumetric logic.
    ...
    def __init__(
        ...
        """Initialize PAX door lintels with frame and element ids.
...
def build_door_lintel(df: pd.DataFrame, **kwargs: float | bool) -> WeightFunction:
    """Build the PAX door lintel weight function.

    Parameters
    ----------
    df : pd.DataFrame
        Parsed primitive rows produced by ``load_door_lintel``.
    **kwargs : float | bool
    ...
    -------
    WeightFunction
        Weight function populated with all PAX door lintel primitives.
    """
    return DoorLintel("PAX_DOOR_LINTEL", df, kwargs).build()
