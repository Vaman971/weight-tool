"""PAX door frame component builders.
...
class DoorFrames(GenericVolumetricBeam):
    """Build PAX door frames with shared volumetric logic.
    ...
    def __init__(
        ...
        """Initialize PAX door frames with frame and element ids.
...
def build_door_frames(df: pd.DataFrame, **kwargs: float | bool) -> WeightFunction:
    """Build the PAX door frame weight function.

    Parameters
    ----------
    df : pd.DataFrame
        Parsed primitive rows produced by ``load_door_frames``.
    **kwargs : float | bool
    ...
    -------
    WeightFunction
        Weight function populated with all PAX door frame primitives.
    """
    return DoorFrames("PAX_DOOR_FRAMES", df, kwargs).build()
