"""PAX door sill component builders.
...
class DoorSill(GenericVolumetricBeam):
    """Build PAX door sills with shared volumetric logic.
    ...
    def __init__(
        ...
        """Initialize PAX door sills with frame and element ids.
...
def build_door_sill(df: pd.DataFrame, **kwargs: float | bool) -> WeightFunction:
    """Build the PAX door sill weight function.

    Parameters
    ----------
    df : pd.DataFrame
        Parsed primitive rows produced by ``load_door_sill``.
    **kwargs : float | bool
    ...
    -------
    WeightFunction
        Weight function populated with all PAX door sill primitives.
    """
    return DoorSill("PAX_DOOR_SILL", df, kwargs).build()
